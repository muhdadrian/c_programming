/*
This program presents a menu of choices (three different decades), gets the user's choice, and then presents a secondary menu (sports, entertainment, and politics). When the user makes her second choice, it prints a list of key information from that specific decade in that specific category. 
*/
...
int main()
{
	// Despite being a long program, you only need two vars:
	...
	// The top-menu choice and the switch statement that makes the resulting information appear are encased in a do-while loop that ensures one of the 4 menu choices are made
	...
	do
	{
		...
		switch(choice1)
		{
			// In the first case, the user picked the 1980s. Now it time to see what specific info they need.
			...
			// The section for when the user selects the 2000s case (3):
			{
				printf("\n\nWhat would you like to see?\n");
				printf("1. Baseball\n");
				printf("2. The Movies\n");
...
// nesting a switch statements is not a good idea
// with more complex blocks of code, the braces can help keep things clear even though it's not needed after each 'case (expression) :'
// use int or char value in switch because float and double values can't be matched properly
// don't use nested if statements when a switch statement will work instead. 
// switch is clearer statement. 